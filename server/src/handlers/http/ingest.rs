...
pub async fn post_event(req: HttpRequest, body: Bytes) -> Result<HttpResponse, PostError> {
    ...
    if !STREAM_INFO.stream_exists(&stream_name) {
     ...
     // Detect Java log format
     match detect_java_log_format(&body) {
        Ok(detected) if detected => {
            println!("Java log format detected");
            // Additional logic for when Java log format is detected
        },
        Ok(_) => {
            match detect_syslog_format(&body) {
                Ok(detected) if detected => {
                    println!("Syslog format detected");
                    // Additional logic for when Syslog log format is detected
                },
                Ok(_) => {
                    println!("Syslog format not detected");
                },
                Err(e) => {
                    eprintln!("Error detecting log format: {}", e);
...
fn detect_java_log_format(body: &Bytes) -> Result<bool, serde_json::Error> {
    // Try to deserialize the JSON and check for keys associated with Java log format
    let log_entry: Value = serde_json::from_slice(body)?;

    // Check for the presence of keys typically found in Java logs
    let has_required_keys = log_entry.get("timestamp").is_some() &&
                            log_entry.get("level").is_some() &&
                            log_entry.get("logger").is_some() &&
                            log_entry.get("message").is_some();

    Ok(has_required_keys)
}

fn detect_syslog_format(body: &Bytes) -> Result<bool, serde_json::Error> {
    let json: Value = serde_json::from_slice(body)?;
    if let Some(log_message) = json.get("log").and_then(|v| v.as_str()) {
        return Ok(is_valid_syslog_format(log_message));
    }
    Ok(false)
}

fn is_valid_syslog_format(message: &str) -> bool {
    // Check for the expected Syslog format
    // Here, we will check if it starts with a timestamp, like "Jan 29 15:49:48.699"
    let timestamp_regex = regex::Regex::new(r"^\w{3} \d{1,2} \d{2}:\d{2}:\d{2}\.\d{3}:").unwrap();
    timestamp_regex.is_match(message)
}
